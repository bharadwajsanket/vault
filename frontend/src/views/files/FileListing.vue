<template>
  <div>
    ...
    <div v-if="layoutStore.loading" class="skeleton-container">
      <div class="skeleton-header">
        <div class="skeleton-col name"></div>
        <div class="skeleton-col size"></div>
        <div class="skeleton-col date"></div>
      </div>
    ...
    <template v-else>
      ...
      >
        <div class="empty-icon-wrapper">
          <i class="material-icons">{{ emptyStateInfo.icon }}</i>
        </div>
        <h3>{{ emptyStateInfo.message }}</h3>
        <p v-if="authStore.user?.perm.create">
          Drag and drop files here, or use the action buttons above to upload.
        </p>
        <input
          style="display: none"
          type="file"
...
const emptyStateInfo = computed(() => {
  ...
  if (folderName === "developer" || folderName === "dev" || folderName === "code" || folderName === "src") {
    return {
      icon: "code",
      message: "No developer files in this vault."
    };
  } else if (folderName === "documents" || folderName === "docs" || folderName === "doc") {
    return {
      icon: "description",
      message: "No documents in this vault."
    };
  } else if (folderName === "photos" || folderName === "images" || folderName === "pics" || folderName === "media") {
    return {
      icon: "photo_library",
      message: "No photos in this vault."
    };
  }
  
  return {
    icon: "folder_open",
    message: t("files.lonely")
  };
});
