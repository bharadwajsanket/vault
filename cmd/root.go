...
func init() {
	...
	rootCmd.SetVersionTemplate("Vault version {{printf \"%s\" .Version}}\n")

	// Flags available across the whole program
	persistent := rootCmd.PersistentFlags()
	persistent.StringP("config", "c", "", "config file path")
	persistent.StringP("database", "d", "./vault.db", "database path")
...
var rootCmd = &cobra.Command{
	Use:     "vault",
	Aliases: []string{"filebrowser"},
	Short:   "Vault — a personal cloud for developers",
	Long: `Vault CLI lets you create the database to use with Vault, manage your users, and configure the service without the web interface.

If you've never run Vault, you'll need a database file. Vault uses Bolt DB as a single-file store and manages it for you.

For this command, all flags are available as environmental variables, except for "--config", which specifies the configuration file to use.
The environment variables are prefixed by "VAULT_" followed by the flag name in UPPER_SNAKE_CASE. For example, the flag "--disablePreviewResize" is available as VAULT_DISABLE_PREVIEW_RESIZE.
Legacy FB_* environment variables are still supported for compatibility.

If "--config" is not specified, Vault will look for a configuration file named .vault.{json, toml, yaml, yml} in the following directories:
...
**Note:** Only the options listed below can be set via the config file or environment variables. Other configuration options live exclusively in the database and must be set by the "config set" or "config import" commands.
...
If the database path doesn't exist, Vault will enter quick setup mode and bootstrap a new database and user using the provided credentials.`,
