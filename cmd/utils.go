...
func initViper(cmd *cobra.Command) (*viper.Viper, error) {
	...
	// Support legacy FB_* environment variables for compatibility.
	for _, env := range os.Environ() {
		if !strings.HasPrefix(env, "FB_") {
			continue
		}
		...
		key := strings.TrimPrefix(parts[0], "FB_")
	...
	if err := v.ReadInConfig(); err != nil {
		...
		// Fallback to legacy config filename for compatibility.
		if v.ConfigFileUsed() == "" {
...
func withViperAndStore(fn func(cmd *cobra.Command, args []string, v *viper.Viper, store *store) error, options storeOptions) cobraFunc {
	return func(cmd *cobra.Command, args []string) error {
		...
		case !exists && !options.expectsNoDatabase && !options.allowsNoDatabase:
			log.Fatal(path + " does not exist. Please run 'vault config init' first.")
		case !exists && !options.expectsNoDatabase:
			log.Println("WARNING: vault.db can't be found. Initialing in " + strings.TrimSuffix(path, "vault.db"))
		}
